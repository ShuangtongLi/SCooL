...
class Recorder(object):
    ...
    def add_new(self,top1_group,train_loss_group,test_acc_group,test_loss_group,\
        mixing_weight_group, mixing_weight_group_pred, epoch):
        ...
        mixing_matrix_pred = self.my_gather(mixing_weight_group_pred,root=0)
        ...
        if self.rank==0:
            ...
            mixing_matrix_pred=mixing_matrix_numpy(mixing_matrix_pred)
            wandb.log({'mixing_matrix_pred'+str(epoch):mixing_matrix})
            wandb_dict=dict()
            ...
            matplotlib.use('Agg')
            fig=plt.figure()
            plt.imshow(mixing_matrix_pred, cmap='Purples')
            plt.colorbar()
            wandb_dict['mixing_weight_pred_heatmap']=wandb.Image(plt)
            wandb_dict['epoch']=epoch

            wandb_dict=add_wandb_dict(wandb_dict,mixing_matrix,name='mixing_weight')
            wandb_dict=add_wandb_dict(wandb_dict,mixing_matrix_pred,name='mixing_weight_pred')
            '''
